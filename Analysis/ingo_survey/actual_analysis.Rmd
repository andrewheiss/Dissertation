...
survey.countries.clean <- readRDS(file.path(PROJHOME, "Data", "data_processed", 
  ...
  mutate(Q4.4_collapsed =
           fct_recode(Q4.4, NULL = c("Don't know")),
...
df.restriction.reg.issue <- survey.countries.clean %>%
  ...
  mutate(Q4.4_collapsed_clean = fct_recode(Q4.4_collapsed,
                                           Registered = "Yes",
                                           `Not registered` = "No"))

plot.restriction.reg.issue <- prodplot(df.restriction.reg.issue,
                                       ~ Q4.4_collapsed_clean + Q4.17_collapsed +
                                         target.regime.type, 
                                       mosaic("h")) +
  aes(fill=Q4.4_collapsed_clean, linetype=target.regime.type, colour="white") +
