...
plot.hq.map.scale <- ggplot(df.hq.countries, aes(fill=num.ceiling, map_id=Q2.2_iso3)) +
  ...
  expand_limits(x=countries.ggmap$long, y=countries.ggmap$lat) + 
  coord_equal() +
  scale_fill_gradient(low="grey85", high="grey20", breaks=seq(0, 30, 10), 
...
plot.issues.most.clean <- ggplot(df.issues.most.clean,
  ...
  geom_barh(stat="identity") +
  scale_x_continuous(expand=c(0, 0),
                     sec.axis = sec_axis(~ . / df.issues.most.denom,
                                         labels=scales::percent)) +
  scale_fill_manual(values=c("grey80", "grey40"), name=NULL) +
...
activities.order.low <- df.activities %>%
  filter(potential.contentiousness == "Low contention", response == "Always") %>%
  arrange(response, desc(num)) %>% 
  mutate(question = ordered(fct_inorder(question))) %>% select(question)

activities.order.high <- df.activities %>%
  filter(potential.contentiousness == "High contention", response == "Always") %>%
  arrange(response, desc(num)) %>% 
  mutate(question = ordered(fct_inorder(question))) %>% select(question)

df.activities.low <- df.activities %>%
  filter(potential.contentiousness == "Low contention") %>%
  mutate(question = factor(question, levels=levels(activities.order.low$question),
                           ordered=TRUE))

df.activities.high <- df.activities %>%
  filter(potential.contentiousness == "High contention") %>%
  mutate(question = factor(question, levels=levels(activities.order.high$question),
                           ordered=TRUE))

plot.activities.low <- ggplot(df.activities.low, aes(y=num, x=response)) +
  geom_bar(stat="identity", fill="grey40") +
  labs(y="Times selected", x="How often INGO does work",
       title="Low contention INGOs") +
  facet_wrap(~ question, ncol=1) +
  theme_ath() + theme(plot.title=element_text(hjust=0.5))

plot.activities.high <- ggplot(df.activities.high, aes(y=num, x=response)) +
  geom_bar(stat="identity", fill="grey80") +
  labs(y=NULL, x="How often INGO does work",
       title="High contention INGOs") +
  facet_wrap(~ question, ncol=1) +
  theme_ath() + theme(plot.title=element_text(hjust=0.5))

plot.activities.both <- cbind(ggplotGrob(plot.activities.low),
                              ggplotGrob(plot.activities.high))
grid::grid.newpage()
grid::grid.draw(plot.activities.both)

fig.save.cairo(plot.activities.both,
